...
class DashboardAPI {
    ...
    public function getClosureInsights() {
        ...
        try {
            ...
            $recentStmt = $this->db->prepare("SELECT
                    t.title,
                    p.name as project,
                    e.name as employee,
            ...
            error_log("getClosureInsights: Error - " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getDeliveryMetrics() {
        ...
        try {
            $projectStmt = $this->db->prepare("SELECT
                    p.id,
                    p.name,
                    p.client,
                    p.status,
            ...
            $projectStmt->execute();
            $projectsRaw = $projectStmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            foreach ($projectsRaw as $project) {
                ...
                $onTimeQuery = $this->db->prepare("SELECT
                ...
                $onTimeData = $onTimeQuery->fetch(PDO::FETCH_ASSOC);
            ...
            $throughputStmt = $this->db->prepare("SELECT
            ...
            $throughputStmt->execute();
            $throughput = $throughputStmt->fetch(PDO::FETCH_ASSOC) ?: [];
            ...
            error_log("getDeliveryMetrics: Error - " . $e->getMessage());
            return [];
        }
    }
    ...
    public function handleGetRequest($action, $params = []) {
        ...
        switch ($action) {
            ...
            case 'closure_insights':
                return $this->getClosureInsights();

            case 'delivery_metrics':
                return $this->getDeliveryMetrics();
