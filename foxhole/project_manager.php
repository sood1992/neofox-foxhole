...
<body>
    ...
    <div class="container">
        ...
        <div class="delivery-hero" id="delivery-hero">
            <div class="hero-main-panel">
                ...
                <p>Real-time pulse on project throughput, cycle time and portfolio health. Use this snapshot to unblock teams and protect deadlines.</p>
                <div class="hero-metric-grid">
                    <div class="hero-metric-card">
                        <span class="hero-metric-label">Average cycle</span>
                        <span class="hero-metric-value" id="hero-average-cycle">--</span>
                    </div>
                    <div class="hero-metric-card">
                        <span class="hero-metric-label">On-time delivery</span>
                        <span class="hero-metric-value" id="hero-on-time">--</span>
                    </div>
                    <div class="hero-metric-card">
                        <span class="hero-metric-label">Active projects</span>
                        <span class="hero-metric-value" id="hero-active-projects">--</span>
                    </div>
                    <div class="hero-metric-card">
                        <span class="hero-metric-label">Closed this week</span>
                        <span class="hero-metric-value" id="hero-week-completed">--</span>
                        <span style="font-size: 12px; color: var(--gray-500);" id="hero-week-delta"></span>
                    </div>
                </div>
            </div>
            <div class="hero-secondary-panel">
                <h3>At-risk focus</h3>
                <ul class="hero-project-list" id="hero-risk-list">
                    <li class="hero-project-item" style="justify-content: center; color: var(--gray-500);">Loading...</li>
                </ul>
            </div>
        </div>
    ...
    <script>
        ...
        function renderDeliveryHero(delivery) {
            const avgCycleEl = document.getElementById('hero-average-cycle');
            const onTimeEl = document.getElementById('hero-on-time');
            const activeProjectsEl = document.getElementById('hero-active-projects');
            const weekCompletedEl = document.getElementById('hero-week-completed');
            const weekDeltaEl = document.getElementById('hero-week-delta');
            const riskList = document.getElementById('hero-risk-list');

            if (!avgCycleEl || !riskList) return;

            const portfolio = delivery?.portfolio_summary ?? {};
            const throughput = delivery?.throughput ?? {};
            const atRisk = delivery?.at_risk_projects ?? [];

            const avgCycle = portfolio.average_cycle_days;
            avgCycleEl.textContent = typeof avgCycle === 'number' ? `${avgCycle.toFixed(1)}d` : '—';

            const onTime = portfolio.on_time_delivery_rate;
            onTimeEl.textContent = typeof onTime === 'number' ? `${onTime}%` : '—';

            activeProjectsEl.textContent = portfolio.active_projects ?? '0';

            const weekCompleted = throughput.week_completed ?? 0;
            const weekGoal = throughput.week_goal ?? 0;
            const lastWeek = throughput.last_week_completed ?? 0;
            const delta = weekCompleted - lastWeek;
            weekCompletedEl.textContent = weekCompleted;
            weekDeltaEl.textContent = `Goal ${weekGoal} · ${delta >= 0 ? '+' : ''}${delta} vs last week`;

            riskList.innerHTML = '';
            if (Array.isArray(atRisk) && atRisk.length > 0) {
                atRisk.slice(0, 4).forEach(project => {
                    const item = document.createElement('li');
                    item.className = 'hero-project-item';
                    item.innerHTML = `
                        <div>
                            <strong>${project.name || 'Project'}</strong>
                            <div style="font-size: 12px; color: var(--gray-500);">
                                ${project.reason || 'Monitor status'}
                            </div>
                        </div>
                        <span class="hero-chip ${project.overdue_tasks && project.overdue_tasks > 0 ? 'danger' : 'success'}">
                            <i class="fas ${project.overdue_tasks && project.overdue_tasks > 0 ? 'fa-triangle-exclamation' : 'fa-check-circle'}"></i>
                            ${project.overdue_tasks && project.overdue_tasks > 0 ? `${project.overdue_tasks} overdue` : 'On track'}
                        </span>`;
                ...
                item.style.color = 'var(--gray-500)';
                item.textContent = 'All projects are on track';
                riskList.appendChild(item);
            }
        }

        async function loadDeliveryMetrics() {
            const hero = document.getElementById('delivery-hero');
            if (!hero) return;

            try {
                const response = await fetch(`api_enhanced.php?action=delivery_metrics&_t=${Date.now()}`);
                const metrics = await response.json();
                renderDeliveryHero(metrics);
            } catch (error) {
                logError('Failed to load delivery metrics', error);
            }
        }
